...
はい、こんにちは
えっとstudist のコバシ先生えっとディレ今日はディレードからスリット級へ完全移行した旅士ということで発表させていただきます
よろしくお願いします
えーはい
えっと今回唯一のかレルズソリッド系の話です
良い人とわけですよね
間違えた
はい
えっと僕なんですけども、一応小林と言います
えっとサイストっていう会社であのsreをやってます
で、アカウントはこの通りなんですけども、苗呼ばれると返字できないんで、翔平さんもしくはスロックさんと呼んでもらえると返事します
でえっと僕のエンジニアア歴なんですけども、一応、よいしょ
えっと履歴は2005年からエンジニアやってます
レールズは三から座ってて一応リナックスはフェドラコアなんてもんから触ってたりします
で一応ウェブ2.0711時代からやってました
またはイラストを書いたりしてますね
はい
今日めっちゃ緊張して死ぬんじゃないかと思ってるんですけども、帰ったら最新のエキタブが届いてるんで頑張ります
はい
で、今日はディレイゾとからソリッド級に移行した話をします
で、勿論障害の話もしますし、トランザクションの中で呼び出して大変なことになった話もするので、皆さんそれを回合わせてこの後のプレゼンの要素を見ていただけると面白いかなと思います
はい、で、この件なんですけど、実僕の方で会社の方からインタビューを受けたりとか、あとブログ、会社のテックブログとかに書いたりをしてるんで、もしわ良かったら合わせてご覧ください
はい
で、今日のアジェンダなんですけど、まず僕らが使っているティビズっていうサービスについて、あそれからま前提としてある課題、それからそれをもとにせあのスり時を選んだ話と、どういうふうに移行したのかっていうところと、その最後に障害を起こした話
で、最後にまとめをします
で、僕らが作っているサービスなんですけど、チチビズと言います
これマニュアルで生産性を向上しようで製造後に製造業の人達に主に使ってもらう為のマニュアル作成共有サービスです
で、こいつなんですけども、実はリリース10年以上経っておりまして、一番最初が例ルの321で最近だと僕はこのプロジェクト始めた時は、レイルズ7.1でした
で、あのどうやって購入人はみんな大丈夫だと思うんですけども、非読期処理の簡単なおされだけすると、だいたいウェブ側でユーザーが操作します
で、データベースに処理が言及されます
で、ユーザーにレスレスポンスが返されて、最後に非同期圧迫ックエンドが実際の処理を行なって、処理結果をデーターベースに保存して終わり、これが非同期書ですね
で、僕らの作っているサービスは結構重要な処理、非同期処理が多くて、あの非同期処理基盤を通して色んな処理が行なわれていますと
で、まずはなんでスリッチ級に変えるのっていうところで課題パートの話をするんですけども、まずは七年前ですね
えっとこちらはうちの上司になります
あのすごいデッドロックとかで苦しんでいたのがまスラック上で分かります
で、今回大切なディレイドジョブディレイドで、ディレイドジョブっていうのは昔でディレードはその進化系みたいなプロダクトですね
これどちらもデータベースをキューとする非同期バックエンドで、アクティブブからを使います
ただこのつの課題があって、やっぱりやりたいことっていうのは僕らの中であって、でまやっぱり大きく分けて三つありました
でパフォーマンスキャラビリティとやっぱ柔軟にジョブ管理したいですよねっていうのと、開発運用もうちょっと効率化したよねってところ
でパフォーマンスcビリティだと線形スケルアウトできるようにしたいっていうのと、やっぱ処理できる職の数を一気に増やしたいですと
で、次にやっぱりリリソース有効活用したいですよねっていうところ
やっぱり優先的なところで言うと、優先度を定義したらまちゃんと優先順位にやってほしいですっていうのと、やっぱりジョブごとにキューと分けたいですと
そして定期実行処理求めたいですみたいな
どうせなら
やっぱ開発運用だとアクティブジョブ移行したいですよねって話があり、ディレード専用の書き方っていうのがあるんですね
で、それとアクティブシャブが混合材してたんで、それをアクティアクティブ上型に移動したいですと
まーやっぱりエルズウェイのスたいよねって
で、これから一応一番大きな課題だったのが、この綺麗に線形skラウトできるようにしたいって話ですね
この綺麗にっていうのがポイントです
で、まずはなんでこれ不可能だったのって話があって、で、これを説明する前にまず非同期処理っていうのを二種類あって、まずジョブとして長時間の処理をしてもらうもの
で、次にとりあえず非同期にするけど、とりあえずさっさと処理して欲しいものっていうのがあって、まず今回多分ターゲットになるのはこのbの方
とりあえず非同期にするけどさっさと承認してがいいというもの
で、不可能問題はちょっと本屋さんで例えします
線形スキルウトができない話っていうのはまず本屋がありますで、あまりに本が売れるんで、ちょっと店さんにしましたよ
で、じゃあ二人にしたから店長さんからしたらお客さんを捌くスピードも2倍になりますよねって言うんですけど、ところが、本がたくさん売れるから店員さんを二人にしたにもかかわらず、電卓が1個しかなかったら、社にお客さんをそばスキードは実は2倍になってないよねって言って
これが線形スキルと不可能問題の例えです
はい
で、じゃあこれをちょっと技術的な話にすると、まずは僕らは自分がいっぱいあるので処理したいですと
で、処理ワーカーを開せば処理はもっと裁けるはず
ただそこで処理を実行すると、ディレイドは一台のマーカーが取得している間、取得が赤赤待つという現象が発生することがあります
これが線形スキラーとの妨げになります
で、もうちょっと不可能な問題はもうちょっと噛み砕いて説明すると、このジョブテーブルっていうのはついにジョブabcが吸引されていますと
で、ワーカーaワーカーbがあるんですけども、こいつらが開いてるジョブをひとつロックして俺にくれっていう話をジョブテーブルにするんですけども、この時にトランザクションが開始されて、フォーアップデートっていうsqlが流れます
それでこれトランザクションコミットがされるまでにワーカーbはロックされてるから、じゃあ直後取得するのは待ちましょうと
でそうなるとどうなるかっていうと、トランザクションがコミットされるまでにジョブbはワカビかワびや常備はとにかず街ワーカーになってしまって、結果としてこのワーカーabcって増やしていっても奇麗に線形スキルアトしてくれない
これが問題です
でその上で選定するんですけども、で、検証としては、ま非同期処理としてはアクティブジョブを使ってるものこれはありました
で、もう一つ問題がディレード側で、ディレードジブの名残りで、クラスネーム.ディレイ.エグゼクっていう.ディレイメソッドっていうものがありましたと
で、こいつらをい何とかしたい
定期処理実行がメンバーっていう自分がみんな懐かしがあると思うんですけども、それからデプロイジにクーロンタブを作成するやつ
で、やっぱジョブの級としてはディレードなんでデータベースを使ってます
この時前ケール八でしっていうか今もそうですね
はい
で、srチームで合理したんですけども、一応運用対象は増やしたくありませんと
で、できたら減らすこともできるといいよねっていう話をしました
はい
でなんかどっかで見たことありますね
はい
2024のレイルズ会議オンレルスでやってたやつですそれで、やっぱ比較観点の一例としてなんですけども、あのやっぱ線形スクラウとさっき話したやつですね
これができるといいですねっていうのと、やっぱ新しいエダブレスソスは不要であること
で、用途に応じて複数急管理ができるといいよねっていうのはもともとありました
やっぱもちろん最初の話した通りなんですけども、データベースロックでパフォーマンスが劣化しないで、ジョブのトランザクションの呼び出しが対応してて、やっぱレイルスと親和性は高く、定期実行基盤がある
まー無料に使いたい
サイドキックプロなんですけど、これも検証したんですけど、全体の五つぐらいしか目指してくれなかったですね
で、サイドキックプロいいところいっぱいあるんですよ
で、やっぱ強力なバッチ図機能とかはやっぱすごい良かったなと思ってて、これたくさん飲食ひとまとめ実行で大きなデータも安心安全処理できること
ですね
でやっぱジョブが失敗しても全然心配いらない
ところが大きくて、やっぱり複数のトランザクションで使えるジャブフェーラーーズだったりとか、それらが自動的に理解するリアブルキズとか
やっぱスケジュール機能はやっぱ最強で、実行処を細かく制御できたりするっていうところが良かったですね
で、やっぱ辞書事情を調べた時にサイドキックプロはやっぱり採用できなかったなっていうのがあって、やっぱadベース一ソース神経追加しなきゃいけないよねバレキだったりレディスだったりのが必要
で、秘めたる力を完璧にこなすには、やっぱりアクティブショブショブ常にやる大量のアクティブジョブバッチの書き換え必要だよねっていうことで、やっぱアクティブジョブだとすべての機能を使い切れませんね
まやっぱり有料でしたっていうのがあります
はい
で、そういう時は一体だいたい全部満たしてくれるよねっていうところで、で、ソリッド級のいいところなんですけども、まやっぱり線形スケラウと大得意ですね
もう本当に行うジョブを実行するワーカーを増やせば増やすだけ、パフォーマンスが上がっていって、ロックを最小限する設計になっているので、やっぱパフォーマンススキャアラビティーの向上っていう観点で、僕らが下題したところは全部満たされていましたっていうのが一番良かったですと
はい
でやっぱいいところパートツーなんですけども、あの運用インスタクションの効率化ですね
はい
で、これ、既存のオーロのインスタンスに対して、新しくクリエイトデータをすればオッケーだったんで新しいインフラリソースの追加が不要っていうところがやっぱすごいき方です
どうなってるかと持ち物が増えないっていうのが最強っていうのんですね
まやっぱりいいところさんなんですけども、やっぱりアクティブジョブバックエンドってところで、やっぱ作っている人たちがレイルの人たちなんですよね
これが本当に強かったです
ただやっぱこいつ物足りないところがあって、あの監視機能がないんですよね
はい
これはやっぱ自前で何とかしてくださいよねって話だったんで、これはちょっと後述します
とりあえずその時のバーと見た感じ、やりたいことは全部解決してくれそうだったので、このまま水時球で行きましょうという話を技術的に例えてきました
で、こっから行ってもいいんですけど、ちょっと内部実装の話をしないといけないので、ここちょっと挟みます
で、そういう時の内部実装の理解が必要なんですけど、お茶はなんで内部実装をしなきゃいけないのっていうことは、まず監視しなきゃいけないです
リリースしたら
で、パフォーマンスチューニングをしなきゃいけないですし、やっぱ障害早い子ってやっぱ発生するので、その時のトラブル対応とかの時にやっぱ内部知っておくと楽です
で、やっぱり知っておくのはデータベース構成を把握して、こういうクエリオを投げればこういうメトリックスが出てきますと
で、パフォーマンスだと直後処理する振りを理解しておけば、あのワーカープロセスとかの調節ができるので、そこできるようにしておく必要がありますと
ですやっぱり障害対応だとそのフローを理解しておくとトラブル不全にくげたりするので、知っておきましょうっていうことですね
ってなった時にデータベースの話を行きます
それリとは複数のテーブルで出来上がってますと
でこれ全体図ざっくり作ったんですけど、これ本当にざっくりなんで、本当は間にいくつかテーブルがあったりします
で、それと救助物ってやつがジョブが言及されると入るテーブルになります
その後、きこの黄色いやつがジョブがスケジュールされると、要は何時何分に失行しますってなると、スケジュールエグゼキューションのところに入ります
で、じゃあいよいよレディと実行しますよっていうのが入ると、このレディエグゼキューションズってここに入り、じゃあ実行中ですってなると、クライ
...
設定カラムの追加 -> enum を string key にしている
