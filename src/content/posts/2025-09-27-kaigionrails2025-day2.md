...
できるだけRails way に載せたいのわかる

A. 長時間処理をさせたいもの
B. 非同期だけどなるはやで処理して欲しいもの
...
はい、こんにちは。えっとstudist のコバシ先生えっとディレ今日はディレードからスリット級へ完全移行した旅士ということで発表させていただきます。よろしくお願いします。えーはい。えっと今回唯一のかレルズソリッド系の話です。良い人とわけですよね。間違えた。はい。えっと僕なんですけども、一応小林と言います。えっとサイストっていう会社であのsreをやってます。で、アカウントはこの通りなんですけども、苗呼ばれると返字できないんで、翔平さんもしくはスロックさんと呼んでもらえると返事します。でえっと僕のエンジニアア歴なんですけども、一応、よいしょ。えっと履歴は2005年からエンジンニアやってます。レールズは三から座ってて一応リナックスはフェドラコアなんてもんから触ってたりします。で一応ウェブ2.0711時代からやってました。またはイラストを書いたりしてますね。はい。今日めっちゃ緊張して死ぬんじゃないかと思ってるんですけども、帰ったら最新のエキタブが届いてるんで頑張ります。はい。で、今日はディレイゾとからソリッド級に移行した話をします。で、勿論障害の話もしますし、トランザクションの中で呼び出して大変なことになった話もするので、皆さんそれを回合わせてこの後のプレゼンの要素を見ていただけると面白いかなと思います。はい、で、この件なんですけど、実僕の方で会社の方からインタビューを受けたりとか、あとブログ、会社のテックブログとかに書いたりをしてるんで、もしわ良かったら合わせてご覧ください。はい。で、今日のアジェンダなんですけど、まず僕らが使っているティビズっていうサービスについて、あそれからま前提としてある課題、それからそれをもとにせあのスり時を選んだ話と、どういうふうに移行したのかっていうところと、その最後に障害を起こした話。で、最後にまとめをします。で、僕らが作っているサービスなんですけど、チチビズと言います。これマニュアルで生産性を向上しようで製造後に製造業の人達に主に使ってもらう為のマニュアル作成共有サービスです。で、こいつなんですけども、実はリリース10年以上経っておりまして、一番最初が例ルの321で最近だと僕はこのプロジェクト始めた時は、レイルズ7.1でした。で、あのどうやって購入人はみんな大丈夫だと思うんですけども、非読期処理の簡単なおされだけすると、だいたいウェブ側でユーザーが操作します。で、データベースに処理が言及されます。で、ユーザーにレスレスポンスが返されて、最後に非同期圧迫ックエンドが実際の処理を行なって、処理結果をデーターベースに保存して終わり、これが非同期書ですね。で、僕らの作っているサービスは結構重要な処理、非同期処理が多くて、あの非同期処理基盤を通して色んな処理が行なわれていますと。で、まずはなんでスリッチ級に変えるのっていうところで課題パートの話をするんですけども、まずは七年前ですね。えっとこちらはうちの上司になります。あのすごいデッドロックとかで苦しんでいたのがまスラック上で分かります。で、今回大切なディレイドジョブディレイドで、ディレイドジョブっていうのは昔でディレードはその進化系みたいなプロダクトですね。これどちらもデータベースをキューとする非同期バックエンドで、アクティブブからを使います。ただこのつの課題があって、やっぱりやりたいことっていうのは僕らの中であって、でまやっぱり大きく分けて三つありました。でパフォーマンスキャラビリティとやっぱ柔軟にジョブ管理したいですよねっていうのと、開発運用もうちょっと効率化したよねってところ。でパフォーマンスcビリティだと線形スケルアウトできるようにしたいっていうのと、やっぱ処理できる職の数を一気に増やしたいですと。で、次にやっぱりリリソース有効活用したいですよねっていうところ。やっぱり優先的なところで言うと、優先度を定義したらまちゃんと優先順位にやってほしいですっていうのと、やっぱりジョブごとにキューと分けたいですと。そして定期実行処理求めたいですみたいな。どうせなら。やっぱ開発運用だとアクティブジョブ移行したいですよねって話があり、ディレード専用の書き方っていうのがあるんですね。で、それとアクティブシャブが混合材してたんで、それをアクティアクティブ上型に移動したいですと。まーやっぱりエルズウェイのスたいよねって。で、これから一応一番大きな課題だったのが、この綺麗に線形skラウトできるようにしたいって話ですね。この綺麗にっていうのがポイントです。で、まずはなんでこれ不可能だったのって話があって、で、これを説明する前にまず非同期処理っていうのを二種類あって、まずジョブとして長時間の処理をしてもらうもの。で、次にとりあえず非同期にするけど、とりあえずさっさと処理して欲しいものっていうのがあって、まず今回多分ターゲットになるのはこのbの方。とりあえず非同期にするけどさっさと承認してがいいというもの。で、不可能問題はちょっと本屋さんで例えします。線形スキルウトができない話っていうのはまず本屋がありますで、あまりに本が売れるんで、ちょっと店さんにしましたよ。で、じゃあ二人にしたから店長さんからしたらお客さんを捌くスピードも2倍になりますよねって言うんですけど、ところが、本がたくさん売れるから店員さんを二人にしたにもかかわらず、電卓が1個しかなかったら、社にお客さんをそばスキードは実は2倍になってないよねって言って。これが線形スキルと不可能問題の例えです。はい。で、じゃあこれをちょっと技術的な話にすると、まずは僕らは自分がいっぱいあるので処理したいですと。で、処理ワーカーを開せば処理はもっと裁けるはず。ただそこで処理を実行すると、ディレイドは一台のマーカーが取得している間、取得が赤赤待つという現象が発生することがあります。これが線形スキラーとの妨げになります。で、もうちょっと不可能な問題はもうちょっと噛み砕いて説明すると、このジョブテーブルっていうのはついにジョブabcが吸引されていますと。で、ワーカーaワーカーbがあるんですけども、こいつらが開いてるジョブをひとつロックして俺にくれっていう話をジョブテーブルにするんですけども、この時にトランザクションが開始されて、フォーアップデートっていうsqlが流れます。それでこれトランザクションコミットがされるまでにワーカーbはロックされてるから、じゃあ直後取得するのは待ちましょうと。でそうなるとどうなるかっていうと、トランザクションがコミットされるまでにジョブbはワカビかワびや常備はとにかず街ワーカーになってしまって、結果としてこのワーカーabcって増やしていっても奇麗に線形スキルアトしてくれない。これが問題です。でその上で選定するんですけども、で、検証としては、ま非同期処理としてはアクティブジョブを使ってるものこれはありました。で、もう一つ問題がディレード側で、ディレードジブの名残りで、クラスネーム.ディレイ.エグゼクっていう.ディレイメソッドっていうものがありましたと。で、こいつらをい何とかしたい。定期処理実行がメンバーっていう自分がみんな懐かしがあると思うんですけども、それからデプロイジにクーロンタブを作成するやつ。で、やっぱジョブの級としてはディレードなんでデータベースを使ってます。この時前ケール八でしっていうか今もそうですね。はい。で、srチームで合理したんですけども、一応運用対象は増やしたくありませんと。で、できたら減らすこともできるといいよねっていう話をしました。はい。でなんかどっかで見たことありますね。はい。2024のレイルズ会議オンレルスでやってたやつですそれで、やっぱ比較観点の一例としてなんですけども、あのやっぱ線形スクラウとさっき話したやつですね。これができるといいですねっていうのと、やっぱ新しいエダブレスソスは不要であること。で、用途に応じて複数急管理ができるといいよねっていうのはもともとありました。やっぱもちろん最初の話した通りなんですけども、データベースロックでパフォーマンスが劣化しないで、ジョブのトランザクションの呼び出しが対応してて、やっぱレイルスと親和性は高く、定期実行基盤がある。まー無料に使いたい。サイドキックプロなんですけど、これも検証したんですけど、全体の五つぐらいしか目指してくれなかったですね。で、サイドキックプロいいところいっぱいあるんですよ。で、やっぱ強力なバッチ図機能とかはやっぱすごい良かったなと思ってて、これたくさん飲食ひとまとめ実行で大きなデータも安心安全処理できること。ですね。でやっぱジョブが失敗しても全然心配いらない。ところが大きくて、やっぱり複数のトランザクションで使えるジャブフェーラーーズだったりとか、それらが自動的に理解するリアブルキズとか。やっぱスケジュール機能はやっぱ最強で、実行処を細かく制御できたりするっていうところが良かったですね。で、やっぱ辞書事情を調べた時にサイドキックプロはやっぱり採用できなかったなっていうのがあって、やっぱadベース一ソース神経追加しなきゃいけないよねバレキだったりレディスだったりのが必要。で、秘めたる力を完璧にこなすには、やっぱりアクティブショブショブ常にやる大量のアクティブジョブバッチの書き換え必要だよねっていうことで、やっぱアクティブジョブだとすべての機能を使い切れませんね。まやっぱり有料でしたっていうのがあります。はい。で、そういう時は一体だいたい全部満たしてくれるよねっていうところで、で、ソリッド級のいいところなんですけども、まやっぱり線形スケラウと大得意ですね。もう本当に行うジョブを実行するワーカーを増やせば増やすだけ、パフォーマンスが上がっていって、ロックを最小限する設計になっているので、やっぱパフォーマンススキャアラビティーの向上っていう観点で、僕らが下題したところは全部満たされていましたっていうのが一番良かったですと。はい。でやっぱいいところパートツーなんですけども、あの運用インスタクションの効率化ですね。はい。で、これ、既存のオーロのインスタンスに対して、新しくクリエイトデータをすればオッケーだったんで新しいインフラリソースの追加が不要っていうところがやっぱすごいき方です。どうなってるかと持ち物が増えないっていうのが最強っていうのんですね。まやっぱりいいところさんなんですけども、やっぱりアクティブジョブバックエンドってところで、やっぱ作っている人たちがレイルの人たちなんですよね。これが本当に強かったです。ただやっぱこいつ物足りないところがあって、あの監視機能がないんですよね。はい。これはやっぱ自前で何とかしてくださいよねって話だったんで、これはちょっと後述します。とりあえずその時のバーと見た感じ、やりたいことは全部解決してくれそうだったので、このまま水時球で行きましょうという話を技術的に例えてきました。で、こっから行ってもいいんですけど、ちょっと内部実装の話をしないといけないので、ここちょっと挟みます。で、そういう時の内部実装の理解が必要なんですけど、お茶はなんで内部実装をしなきゃいけないのっていうことは、まず監視しなきゃいけないです。リリースしたら。で、パフォーマンスチューニングをしなきゃいけないですし、やっぱ障害早い子ってやっぱ発生するので、その時のトラブル対応とかの時にやっぱ内部知っておくと楽です。で、やっぱり知っておくのはデータベース構成を把握して、こういうクエリオを投げればこういうメトリックスが出てきますと。で、パフォーマンスだと直後処理する振りを理解しておけば、あのワーカープロセスとかの調節ができるので、そこできるようにしておく必要がありますと。ですやっぱり障害対応だとそのフローを理解しておくとトラブル不全にくげたりするので、知っておきましょうっていうことですね。ってなった時にデータベースの話を行きます。それリとは複数のテーブルで出来上がってますと。でこれ全体図ざっくり作ったんですけど、これ本当にざっくりなんで、本当は間にいくつかテーブルがあったりします。で、それと救助物ってやつがジョブが言及されると入るテーブルになります。その後、きこの黄色いやつがジョブがスケジュールされると、要は何時何分に失行しますってなると、スケジュールエグゼキューションのところに入ります。で、じゃあいよいよレディと実行しますよっていうのが入ると、このレディエグゼキューションズってここに入り、じゃあ実行中ですってなると、クライ
Yes, hello. Um, Sadist Dr. Kobashi, um, today I'd like to announce that I've completely transitioned from delayed to slit class as a traveler. Thank you very much.Uhh...Yes.Um, this time it's the only story about Karel's solid system.With a good personI see. I made a mistake.Yes. Um, as for me, I go by Kobayashi. I work as an SRE at a company called Cybozu. This is my account, but I won't respond if called by my first name. I'll respond if you call me Shohei-san or Slock-san.So, um, regarding my engineering experience, well, here goes.Um, my history is that I've been working as an engineer since 2005. I've been using Rails since version 3, and I've been working with Linux since Fedora Core. And I've been in the web industry since the Web 2.0 era.Or I'm also drawing illustrations. Yes. Today I'm so nervous I feel like I might die, but when I get home, the latest Ekitab will have arrived, so I'll do my best. Yes. And today, I'll talk about transitioning from Delay-zo to Solid class. Of course, I'll also talk about issues, and about how things got complicated when called within a transaction, so if you all put that together and look at the elements of the presentation after this, I think it might be interesting.Yes, so regarding this matter, I've actually been interviewed by the company and have written for the company's tech blog and such, so please feel free to check those out as well. Yes. Now, for today's agenda, we'll first talk about the TBiz service we're using, then the underlying challenges, followed by why we chose that timing based on those factors, how we made the transition, and finally the story of when we encountered issues. Lastly, we'll summarize everything.So, the service we're creating is called ChichiBiz. It's a manual creation and sharing service mainly used by manufacturing industry people to improve productivity after production. Actually, this thing has been released for over 10 years, with the very first version being Rails 3.2.1, and when I started this project recently, it was Rails 7.1.So, while I think all the buyers will be fine, if they just do a simple non-reading period processing,Basically, the user operates on the web side. Then, processing is mentioned in the database. Then, a response is returned to the user, and finally, the asynchronous backend performs the actual processing, saves the processing results to the database, and that's it. This is asynchronous processing.So, the service we're creating has a lot of important processes, many asynchronous processes, and various operations are being performed through that asynchronous processing foundation, you see.So, first, we'll talk about the challenge part of why we're changing to the switch class. It was seven years ago. Um, this is my boss. He was really struggling with deadlocks and such, which you can see on Slack.So, for the important delayed job delayed this time, delayed job is an old product and delayed is like its evolved version. Both of these use the database as a queue for asynchronous backend, we use from active.So we had these challenges, and there were things we wanted to do. Broadly speaking, there were three main areas. We wanted to improve performance capability, manage jobs more flexibly, and make development and operations more efficient. For performance capability, we wanted to enable linear scale-out and dramatically increase the number of jobs we could process, you know.So, next is the part about wanting to effectively utilize resources, right? When it comes to priorities, once we define them, we want them to be properly followed. Also, we want to separate queues for each job. And then we want scheduled execution processing, it seems like. Might as well.So there was talk about wanting to transition to active jobs in development operations, and there's a specific way to write for delayed jobs. And since active jobs were mixed in, we want to move them to an active upper mold. Well, as expected, Elsewhey's style, right?So, the biggest challenge we faced was wanting to make this linear sk route clean and neat. The key point here is "clean and neat."So, first there's the question of why this was impossible, and before explaining that, there are two types of asynchronous processing. First, there's the one that handles long-term processing as a job. Then, there's the one that's made asynchronous but you want it processed quickly anyway. The target this time is probably the latter, b. It's the one that's made asynchronous but you want it approved quickly. Now, I'll explain the impossibility problem using a bookstore example. The story of why linear skill-out is impossible starts with a bookstore, and because too many books are selling, the storemade it like that.So, you'd think that with two people, the manager would expect the speed of serving customers to double, right? But, even though they increased to two staff members because a lot of books were selling, if there was only 1 calculator,I told the customer at the company that the soba skill actually hasn't doubled, right? This is an example of linear skill and the impossibility problem. Yes. So, to make this a bit more technical, first, we have a lot of ourselves, so we want to process.So, if we open processing workers, the processing should be more scalable. However, when executing the process there, a phenomenon may occur where acquisition waits while a single marker is being acquired. This can interfere with linear scaling.So, to explain the impossible problem a bit more simply, this job table finally has jobs a, b, and c absorbed. Now, we have worker a and worker b, and they're telling the job table to lock one open job and give it to them. At this time, a transaction starts, and an SQL "for update" flows. Until this transaction is committed, worker b is locked, so let's wait immediately after acquiring.So what happens then is that before the transaction is committed, job b becomes a street worker regardless of whether it's wakabi or wabi or regular, and as a result, even if you increase these workers abc, they won't scale linearly in a clean way. This is a problem.And then we select based on that,And, for verification,As for asynchronous processing, we were using active jobs. Then, there was another issue on the delayed side, a remnant of delayed jobs, with a delay method called ClassName.delay.exec. We wanted to do something about these. I think everyone nostalgically remembers the periodic task execution as a member, and the one that creates cron tabs for deployment. As for the job class, it's delayed, so we're using a database. At that time, it was scale 8...I mean, it's still like that now, right? Yes. So, we rationalized with the SR team, but we don't want to increase the operational targets for now. And we talked about how it would be nice if we could reduce them if possible.Yes. And I feel like I've seen it somewhere before. Yes. 2024's RailsThat's the one we did on conference onlineSo, as an example of a comparison point, you know, it's that linear scrow thing we talked about earlier. It would be nice if we could do that, and of course, not needing new edable resources. And being able to manage multiple urgencies according to the use case was something we originally had. Of course, as I mentioned at the beginning, it's about not degrading performance with database locks, supporting job transaction calls, having high compatibility with Rails, and having a periodic execution foundation. Well... want to use it for free.So, about Sidekick Pro, I tested it too, but it only targeted about five things overall. But you know, Sidekick Pro has a lot of good points. And,I really think the powerful batch diagram function was incredibly good, as it allows us to process a lot of food and drink data together safely and securely.


[Railsだからできる、例外業務に禍根を残さない設定設計パターン / ei.ei.eiichi | Kaigi on Rails 2025](https://kaigionrails.org/2025/talks/ei-ei-eiichi/#day2)
